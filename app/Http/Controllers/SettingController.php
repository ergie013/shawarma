...
class SettingController extends Controller
{
    ...
    public function saveSettings(Request $request, Factory $cache)
    {
        ...
        if ($request->hasFile('logo')) {
            ...
            $filename = 'logo' . '.png' . '.jpg' . '.jpeg';
            $smallFile = 'logo-sm' . '.png'. '.jpg' . '.jpeg';
            Image::make($image)->resize(320, 89)->save(base_path('/assets/img/logos/' . $filename), 100, 'png' . '.jpg' . '.jpeg');
            Image::make($image)->resize(120, 33)->save(base_path('/assets/img/logos/' . $smallFile), 100, 'png' . '.jpg' . '.jpeg' );
            $setting->value = $filename . '?v=' . time() . str_random(5);
            $setting->save();
        }
